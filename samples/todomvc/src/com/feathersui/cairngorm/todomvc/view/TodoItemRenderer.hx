...
import com.feathersui.cairngorm.todomvc.control.RemoveTodoItemEvent;
...
class TodoItemRenderer extends ItemRenderer {
	...
	private function commitUserChanges():Void {
		...
		var cgEvent = new EditTodoItemEvent(this.todoItem, newText);
	...
	private function completedCheck_changeHandler(event:Event):Void {
		...
		// broadcast the cairngorm event
		var cgEvent = new ToggleTodoItemCompletedEvent(this.todoItem);
		CairngormEventDispatcher.getInstance().dispatchEvent(cgEvent);
	}

	private function deleteButton_triggerHandler(event:TriggerEvent):Void {
		// broadcast the cairngorm event
		var cgEvent = new RemoveTodoItemEvent(this.todoItem);
		CairngormEventDispatcher.getInstance().dispatchEvent(cgEvent);
	}
