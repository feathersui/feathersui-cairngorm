...
class ToggleCompleteAllCommand implements ICommand {
	private var model = TodosModelLocator.getInstance();

	public function execute(cgEvent:CairngormEvent):Void {
		var hasIncomplete = Lambda.exists(this.model.todosListDP.array, item -> !item.completed);
		for (todoItem in this.model.todosListDP.array) {
			todoItem.completed = hasIncomplete;
		}
		this.model.todosListDP.updateAll();
	}
}
